...
export async function applyAccountProviderConfig(deps: AccountProviderConfigDeps): Promise<void> {
  const effectiveLayer = deps.layer ?? Layer.mergeAll(Account.defaultLayer, modelRepoDefaultLayer)

  const result = await Effect.runPromiseExit(
    (doApplyAccountProviderConfig(deps) as Effect.Effect<void, never, never>).pipe(Effect.provide(effectiveLayer)),
  )
