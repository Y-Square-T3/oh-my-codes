...
const mapAuditRepoError = (message: string) =>
  Effect.catchAll((cause: unknown) => {
    ...
    return Effect.fail(
      Object.assign(err, { _tag: "AuditRepoError", message }) as AuditRepoError,
    )
  })
      ...
      Effect.tryPromise({
        try: () => {
          ...
          return database.db
            ...
            .set({ pushed: 1 })
            .where(
              sql`${tokenUsages.id} IN (${sql.join(ids.map((id) => sql`${id}`), sql`, `)})`,
            )
        },
      ...
      Effect.tryPromise({
        try: async () => {
          const result = await database.db
            ...
            .run()
          return database.db.select({ count: sql<number>`count(*)` }).from(tokenUsages).where(eq(tokenUsages.pushed, 1))
        },
