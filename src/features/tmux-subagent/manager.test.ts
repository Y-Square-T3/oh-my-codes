...
const mockPaneExists = mock<(paneId: string) => Promise<boolean>>(
  async () => true,
)
const mockExecuteActions = mock<
  (actions: PaneAction[], ctx: ExecuteContext) => Promise<ExecuteActionsResult>
>(async () => ({
  success: true,
  spawnedPaneId: "%mock",
  results: [],
...
const mockExecuteAction = mock<
  (action: PaneAction, ctx: ExecuteContext) => Promise<ActionResult>
>(async () => ({ success: true }))
...
>(async () => true)
const mockSweepStaleOmoAgentSessions = mock<() => Promise<number>>(
  async () => 0,
)
...
mock.module("../../shared/tmux", () => {
  const {
    isInsideTmux,
    getCurrentPaneId,
  } = require("../../shared/tmux/tmux-utils")
  const {
  ...
  describe("onSessionCreated", () => {
    ...
    describe("spawn failure recovery", () => {
      ...
      test("#given spawn fails without close action #when onSessionCreated fires #then session is enqueued in deferred queue", async () => {
        ...
        mockIsInsideTmux.mockReturnValue(true)
        mockQueryWindowState.mockImplementation(async () => createWindowState())
        mockExecuteActions.mockImplementation(
          async (actions: PaneAction[]) => ({
            success: false,
