...
const serviceError = (message: string, cause?: unknown) => new AccountServiceError({ message, cause })
...
const decodeSchema =
  <A, I>(schema: S.Schema<A, I, never>, errorMsg: string) =>
  (json: unknown) =>
    Effect.try({
...
export interface AccountServiceInterface {
  ...
  readonly activeWithToken: () => Effect.Effect<
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const list = () =>
      repo
        .list()
        .pipe(Effect.catchTag("AccountRepoError", (e) => Effect.fail(serviceError("Failed to list accounts", e.cause))))
    ...
    const token = (accountID: AccountID): Effect.Effect<Option.Option<AccessToken>, AccountError, never> =>
      resolveAccess(accountID).pipe(Effect.map(Option.map((r) => r.accessToken))) as unknown as Effect.Effect<
        Option.Option<AccessToken>,
        AccountError,
        never
      >
    ...
    const remove = (accountID: AccountID) =>
      repo
        .remove(accountID)
        .pipe(
          Effect.catchTag("AccountRepoError", (e) => Effect.fail(serviceError("Failed to remove account", e.cause))),
        )

    const use = (accountID: AccountID, workspaceID: Option.Option<WorkspaceID>) =>
      repo
        .use(accountID, workspaceID)
        .pipe(
          Effect.catchTag("AccountRepoError", (e) => Effect.fail(serviceError("Failed to switch workspace", e.cause))),
        )
