...
export async function getAllSkills(
  ...
  const hasDisabledSkills =
    options?.disabledSkills && options.disabledSkills.size > 0
  ...
  const uniqueBuiltins = builtinSkillsAsLoaded.filter(
    (skill) => !discoveredNames.has(skill.name),
  )
  ...
  if (hasDisabledSkills) {
    allSkills = allSkills.filter(
      (skill) => !options!.disabledSkills!.has(skill.name),
    )
  } else {
