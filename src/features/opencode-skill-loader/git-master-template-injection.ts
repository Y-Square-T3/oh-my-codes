...
export function injectGitMasterConfig(
  ...
  let result = gitEnvPrefix
    ? injectGitEnvPrefix(template, gitEnvPrefix)
    : template
  ...
  return gitEnvPrefix
    ? prefixGitCommandsInBashCodeBlocks(result, gitEnvPrefix)
    : result
}
  ...
  if (commitFooter) {
    const footerText =
      typeof commitFooter === "string"
        ? commitFooter
        : "Ultraworked with [Sisyphus](https://github.com/vibration-autos/oh-my-codes)"
