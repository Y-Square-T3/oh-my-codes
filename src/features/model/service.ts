...
const liftError = <A, E>(effect: Effect.Effect<A, E>): Effect.Effect<A, ModelServiceError> =>
  effect.pipe(Effect.catchAll((e: E) => Effect.fail(new ModelServiceError(String(e), e))))
  ...
  Effect.gen(function* () {
    ...
    const list = (providerId?: string) =>
      Effect.gen(function* () {
        const accountOpt = yield* liftError(
          accountSvc.active().pipe(Effect.catchAll(() => Effect.succeed(Option.none()))),
        )
        ...
        const providers = yield* liftError(
          providerId
            ? repo.listProviders(accountId).pipe(Effect.map((ps) => ps.filter((p) => p.id === providerId)))
    ...
    const refresh = () =>
      Effect.gen(function* () {
        ...
        const tokenOpt = yield* liftError(
          accountSvc.token(accountId).pipe(Effect.catchAll(() => Effect.succeed(Option.none()))),
        )
        ...
        const apiData = yield* Effect.tryPromise({
          try: () => fetchApiJson(account.url, accountId, token),
            ...
            e instanceof ModelApiError ? e : new ModelApiError("GET", `${account.url}/models/api.json`, String(e)),
        })

        const transformedProviders = Object.entries(apiData).map(([id, p]) => transformProvider(id, p, accountId))
    ...
    const clear = (providerId?: string) =>
      Effect.gen(function* () {
        ...
        const filteredModels = providerId ? existingModels.filter((m) => m.providerId === providerId) : existingModels
...
export const defaultLayer = layer.pipe(Layer.provide(Repo.defaultLayer), Layer.provide(Account.defaultLayer))
