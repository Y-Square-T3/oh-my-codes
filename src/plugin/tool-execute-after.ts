  ...
  return async (
    ...
    if (stored) {
      ...
      if (stored.metadata) {
        if (nativeSessionId) {
          log(
            "[tool-execute-after] Native output metadata already includes session linkage; skipping stored metadata overwrite",
            {
              tool: input.tool,
              sessionID: input.sessionID,
              callID: input.callID ?? input.callId ?? input.call_id,
    ...
    if (input.tool === "task") {
      ...
      const sessionId = getMetadataString(output.metadata, [
        "sessionId",
        "sessionID",
        "session_id",
      ])
      ...
      if (
        ...
        log(
          "[tool-execute-after] Stored oracle verification session via attempt match",
          {
            parentSessionID: input.sessionID,
            oracleSessionID: sessionId,
            verificationAttemptId,
        ...
        log(
          "[tool-execute-after] Fallback: stored oracle verification session without attempt match",
          {
            parentSessionID: input.sessionID,
            oracleSessionID: sessionId,
            hasPromptInMetadata: typeof prompt === "string",
    ...
    const runToolExecuteAfterHooks = async (): Promise<void> => {
      ...
      await hooks.contextWindowMonitor?.["tool.execute.after"]?.(
        hookInput,
        output,
      )
