...
describe("createEventHandler - idle deduplication", () => {
  it("#given synthetic idle fires first #when real idle arrives within 500ms #then real idle dispatched", async () => {
    ...
    expect(
      (dispatchCalls[1]?.event.properties as { sessionID?: string } | undefined)
        ?.sessionID,
    ).toBe(sessionId)
  })
    ...
    const eventHandler = createEventHandler({
      ...
      managers: createEventHandlerManagers({
        ...
        tmuxSessionManager: {
          onSessionCreated: async () => {},
          onSessionDeleted: async () => {},
          getTrackedPaneId: (sessionID: string) =>
...
describe("createEventHandler - retry dedupe lifecycle", () => {
  it("re-handles same retry key after session recovers to idle status", async () => {
    ...
    const retryStatus = {
      ...
      attempt: 1,
      message:
        "All credentials for model claude-opus-4-7-thinking are cooling down [retrying in 7m 56s attempt #1]",
      next: 476,
    } as const
      ...
      asEventHandlerInput({
        event: {
          ...
          properties: {
            ...
            error: {
              name: "Error",
              message: "tool_result block(s) that are not immediately",
            },
          },
        },
      }),
