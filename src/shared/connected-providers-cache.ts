...
export function createConnectedProvidersCacheStore(
  ...
  const connectedProvidersCacheStore =
    createJsonFileCacheStore<ConnectedProvidersCache>({
      getCacheDir,
  ...
  const providerModelsCacheStore =
    createJsonFileCacheStore<ProviderModelsCache>({
      getCacheDir,
        ...
        if (provider.models) {
          const modelMetadata = Object.entries(provider.models).map(
            ([modelID, rawMetadata]) => {
              ...
              const normalizedID =
                typeof rawMetadata.id === "string" ? rawMetadata.id : modelID
