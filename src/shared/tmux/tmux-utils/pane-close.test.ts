...
function createDrainSensitiveStream(
  byteLength: number,
  signal: DrainSignal,
...
function createStdoutSensitiveProcess(
  ...
  const stdout = createDrainSensitiveStream(stdoutBytes, {
    onPull: () => resolveDrained(),
  })
...
function resolveWithin<TResult>(
  promise: Promise<TResult>,
  milliseconds: number,
): Promise<TResult | typeof TIMEOUT> {
  return Promise.race([
    promise,
...
describe("closeTmuxPane", () => {
  beforeEach(() => {
    ...
    getTmuxPathMock.mockImplementation(
      async (): Promise<string | undefined> => "tmux",
    )
  })
  ...
  it("#given pane exists #when closeTmuxPane called #then returns true and invokes send-keys + kill-pane in order", async () => {
    ...
    expect(spawnCalls).toEqual([
      {
        command: ["tmux", "send-keys", "-t", "%42", "C-c"],
        options: { stdout: "ignore", stderr: "ignore" },
      },
      {
        command: ["tmux", "kill-pane", "-t", "%42"],
  ...
  it("#given pane already closed by Ctrl+C (kill-pane reports 'can't find pane') #when closeTmuxPane called #then returns true", async () => {
    // given
    const closeTmuxPane = await loadCloseTmuxPane()
    queuedProcesses.push(createProcess(0), {
  ...
  it("#given kill-pane stdout stream waits for drain #when closeTmuxPane called #then returns true once drainer consumes stdout", async () => {
    ...
    queuedProcesses.push(
      createProcess(0),
      createStdoutSensitiveProcess(0, 16 * 1024),
    )
