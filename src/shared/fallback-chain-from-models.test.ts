...
describe("flattenToFallbackModelStrings", () => {
  ...
  it("passes through plain strings", () => {
    expect(
      flattenToFallbackModelStrings(["anthropic/claude-sonnet-4-6"]),
  ...
  it("flattens object without variant", () => {
    expect(
      flattenToFallbackModelStrings([{ model: "openai/gpt-5.4" }]),
...
describe("findMostSpecificFallbackEntry", () => {
  ...
  it("returns prefix match when no exact match exists", () => {
    const chain = [{ providers: ["openai"], model: "gpt-5.4" }]
    const result = findMostSpecificFallbackEntry(
      "openai",
      "gpt-5.4-preview",
      chain,
  ...
  it("returns undefined when no entry matches", () => {
    const chain = [{ providers: ["anthropic"], model: "claude-sonnet-4-6" }]
    expect(
      findMostSpecificFallbackEntry("openai", "gpt-5.4", chain),
    ).toBeUndefined()
  })
