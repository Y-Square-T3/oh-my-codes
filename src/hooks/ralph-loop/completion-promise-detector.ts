...
function extractTranscriptEntryText(entry: TranscriptEntry): string {
  ...
  if (
    entry.tool_output &&
    typeof entry.tool_output === "object" &&
    typeof entry.tool_output.output === "string"
  )
    return entry.tool_output.output
  return ""
}
...
function shouldInspectSessionMessagePart(
  ...
  return (
    promise === ULTRAWORK_VERIFICATION_PROMISE && isOracleVerified(partText)
  )
}
...
function shouldInspectTranscriptEntry(
  ...
  return (
    promise === ULTRAWORK_VERIFICATION_PROMISE && isOracleVerified(entryText)
  )
}
...
export async function detectCompletionInSessionMessages(
  ...
  try {
    ...
    const assistantMessages = (
      scopedMessages as OpenCodeSessionMessage[]
    ...
    for (let index = assistantMessages.length - 1; index >= 0; index -= 1) {
      ...
      for (const part of assistant.parts) {
        ...
        if (
          !shouldInspectSessionMessagePart(part.type, options.promise, partText)
        )
          continue
