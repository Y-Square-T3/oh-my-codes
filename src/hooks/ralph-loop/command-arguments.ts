...
export function parseRalphLoopArguments(
  rawArguments: string,
): ParsedRalphLoopArguments {
  ...
  const completionPromiseQuoted = rawArguments.match(
    /--completion-promise=(["'])(.+?)\1/i,
  )
  const completionPromiseUnquoted = rawArguments.match(
    /--completion-promise=([^\s"']+)/i,
  )
  const completionPromise =
    completionPromiseQuoted?.[2] ?? completionPromiseUnquoted?.[1]
  const strategyMatch = rawArguments.match(/--strategy=(reset|continue)/i)
  const strategyValue = strategyMatch?.[1]?.toLowerCase()

  return {
    prompt,
    maxIterations: maxIterationMatch
      ? Number.parseInt(maxIterationMatch[1], 10)
      : undefined,
    completionPromise,
    strategy:
