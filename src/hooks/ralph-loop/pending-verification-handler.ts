...
async function detectOracleVerificationFromParentSession(
  ...
  try {
    ...
    log(
      `[${HOOK_NAME}] Failed to scan parent session for oracle verification evidence`,
      {
        parentSessionID,
        error: String(error),
      },
...
export async function handlePendingVerification(
  ...
  if (
    matchesParentSession ||
    (verificationSessionID && matchesVerificationSession)
  ) {
    if (!verificationSessionID && state.session_id) {
      const recoveredVerificationSessionID =
        await detectOracleVerificationFromParentSession(
          ctx,
          state.session_id,
          directory,
          apiTimeoutMs,
      ...
      if (recoveredVerificationSessionID) {
        ...
        if (updatedState) {
          log(
            `[${HOOK_NAME}] Recovered missing verification session from parent evidence`,
            {
