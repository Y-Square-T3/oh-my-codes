...
import {
  handleDeletedLoopSession,
  handleErroredLoopSession,
  ...
  return async ({
    ...
    if (event.type === "session.idle") {
      ...
      try {
        ...
        const transcriptPath = completionSessionID
        ...
        if (state.verification_pending) {
          if (!verificationSessionID && matchesParentSession) {
            log(
              `[${HOOK_NAME}] Verification pending without tracked oracle session, running recovery check`,
              {
                sessionID,
                iteration: state.iteration,
              },
