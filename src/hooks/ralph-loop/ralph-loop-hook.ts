...
export function createRalphLoopHook(
  ...
  const getTranscriptPath =
    options?.getTranscriptPath ?? getDefaultTranscriptPath
  ...
  return {
    ...
    startLoop: (sessionID, prompt, loopOptions): boolean => {
      ...
      if (
        !startSuccess ||
        typeof loopOptions?.messageCountAtStart === "number"
      ) {
        return startSuccess
      }
      ...
      ctx.client.session
        ...
        .then((messagesResponse: unknown) => {
          const messageCountAtStart =
            getMessageCountFromResponse(messagesResponse)
