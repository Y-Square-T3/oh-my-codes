...
export async function handleSessionIdle(args: {
  ...
  const effectiveCooldown =
    CONTINUATION_COOLDOWN_MS *
    Math.pow(2, Math.min(state.consecutiveFailures, 5))
  if (
    state.lastInjectedAt &&
  ...
  if (
    resolvedAgentName &&
    skipAgents.some(
      (s) => getAgentConfigKey(s) === getAgentConfigKey(resolvedAgentName),
    )
  ) {
  ...
  if (
    (compactionGuardActive || encounteredCompaction) &&
    !resolvedInfo?.agent
  ) {
    log(
  ...
  const progressUpdate = sessionStateStore.trackContinuationProgress(
    ...
    todos,
    {
      allowActivityProgress: shouldAllowActivityProgress(
        resolvedInfo?.model?.modelID,
      ),
    },
  )
