...
> {
  mock.module("../auto-update-checker/constants", () => ({
    ...
    getUserOpencodeConfigJsonc: () =>
      join(TEST_USER_CONFIG_DIR, "opencode.jsonc"),
  ...
  const cacheModule = await import(
    `../auto-update-checker/cache?test=${importCounter++}`
  )
  mock.restore()
  return cacheModule
}
...
function resetTestCache(): void {
  ...
  mkdirSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-codes"), {
    recursive: true,
  })
  writeFileSync(
...
describe("invalidatePackage", () => {
  ...
  it("invalidates the installed package from the OpenCode cache directory", async () => {
    ...
    expect(
      existsSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-codes")),
    ).toBe(false)
    ...
    expect(bunLock.workspaces?.[""]?.dependencies?.["oh-my-codes"]).toBe(
      "latest",
    )
