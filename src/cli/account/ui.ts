...
import type { AccountID, AccountInfo, AccountWorkspace, Workspace, WorkspaceID } from "../../features/account"

export const intro = (title: string): Effect.Effect<void> => Effect.sync(() => p.intro(pc.cyan(title)))
...
export const logInfo = (message: string): Effect.Effect<void> => Effect.sync(() => p.note(message, pc.blue("info")))

export const logSuccess = (message: string): Effect.Effect<void> =>
  Effect.sync(() => p.note(message, pc.green("success")))

export const logError = (message: string): Effect.Effect<void> => Effect.sync(() => p.note(message, pc.red("error")))
...
export const println = (msg: string): Effect.Effect<void> => Effect.sync(() => console.log(msg))

export const dim = (value: string): string => pc.dim(value)

export const activeSuffix = (isActive: boolean): string => (isActive ? dim(" (active)") : "")
...
export const select = <T>(options: ClackOption<T>[], message: string): Effect.Effect<Option.Option<T>> =>
  Effect.tryPromise(() =>
    p
      .select({
        message,
