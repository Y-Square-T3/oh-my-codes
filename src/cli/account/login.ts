...
import type { AccountID, AccountInfo, WorkspaceID } from "../../features/account"
import { AccountError, defaultLayer, PollResult, PollSuccess, Service as Account } from "../../features/account"
...
export const loginEffect = (url: string): Effect.Effect<void, AccountError, never> =>
  Effect.gen(function* () {
    ...
    if (result._tag === "PollSuccess") {
      ...
      yield* spinner.stop("Error: " + String((result as { cause: unknown }).cause), 1)
    }
  }) as unknown as Effect.Effect<void, AccountError, never>

export async function login(url: string): Promise<number> {
  const result = await Effect.runPromiseExit(loginEffect(url).pipe(Effect.provide(defaultLayer)))
