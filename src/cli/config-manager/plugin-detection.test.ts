...
describe("detectCurrentConfig - single package detection", () => {
    ...
    beforeEach(() => {
        ...
        testOmcConfigPath = join(testConfigDir, "oh-my-codes.json")
    ...
    it("detects both legacy and canonical plugin entries", () => {
        // given
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes@3.11.0"]}, null, 2) + "\n", "utf-8")
    ...
    it("returns false when plugin not present with similar name", () => {
        // given
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes-extra"]}, null, 2) + "\n", "utf-8")
    ...
    it("detects OpenCode Go from the existing omo config", () => {
        // given
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes"]}, null, 2) + "\n", "utf-8")
        writeFileSync(testOmcConfigPath, JSON.stringify({agents: {atlas: {model: "opencode-go/kimi-k2.5"}}}, null, 2) + "\n", "utf-8")
...
describe("addPluginToOpenCodeConfig - single package writes", () => {
    ...
    it("writes canonical plugin entry for new installs", async () => {
        ...
        expect(savedConfig.plugin).toEqual(["oh-my-codes"])
    })
    ...
    it("updates a version-pinned legacy entry to the requested version", async () => {
        // given
        const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-codes@3.16.0")
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes@3.15.0"]}, null, 2) + "\n", "utf-8")
        ...
        expect(result.success).toBe(true)
        const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
        expect(savedConfig.plugin).toEqual(["oh-my-codes@3.16.0"])
        getPluginNameWithVersionSpy.mockRestore()
    })

    it("removes stale legacy entry when canonical and legacy entries both exist", async () => {
        // given
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes", "oh-my-codes"]}, null, 2) + "\n", "utf-8")
    ...
    it("preserves a canonical entry when the same version is re-installed", async () => {
        // given
        const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-codes@3.10.0")
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes@3.10.0"]}, null, 2) + "\n", "utf-8")
        ...
        expect(result.success).toBe(true)
        const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
        expect(savedConfig.plugin).toEqual(["oh-my-codes@3.10.0"])
        getPluginNameWithVersionSpy.mockRestore()
    })

    it("blocks a downgrade for a version-pinned canonical entry", async () => {
        // given
        const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-codes@3.15.0")
        writeFileSync(testConfigPath, JSON.stringify({plugin: ["oh-my-codes@3.16.0"]}, null, 2) + "\n", "utf-8")
