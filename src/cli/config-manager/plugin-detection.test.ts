...
describe("detectCurrentConfig - single package detection", () => {
  ...
  it("returns false when plugin not present with similar name", () => {
    ...
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: ["oh-my-codes-extra"] }, null, 2) + "\n",
      "utf-8",
    )
...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  it("updates a version-pinned legacy entry to the requested version", async () => {
    ...
    ).mockResolvedValue("oh-my-codes@3.16.0")
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: ["oh-my-codes@3.15.0"] }, null, 2) + "\n",
      "utf-8",
    )
  ...
  it("removes stale legacy entry when canonical and legacy entries both exist", async () => {
    ...
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: ["oh-my-codes", "oh-my-codes"] }, null, 2) +
        "\n",
      "utf-8",
    )
  ...
  it("preserves a canonical entry when the same version is re-installed", async () => {
    ...
    const getPluginNameWithVersionSpy = spyOn(
      pluginNameWithVersion,
      "getPluginNameWithVersion",
    ).mockResolvedValue("oh-my-codes@3.10.0")
    writeFileSync(
      testConfigPath,
  ...
  it("blocks a downgrade for a version-pinned canonical entry", async () => {
    ...
    const getPluginNameWithVersionSpy = spyOn(
      pluginNameWithVersion,
      "getPluginNameWithVersion",
    ).mockResolvedValue("oh-my-codes@3.15.0")
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: ["oh-my-codes@3.16.0"] }, null, 2) + "\n",
      "utf-8",
    )
