...
describe("checkCompletionConditions continuation coverage", () => {
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "lineage-only-session" ? "root-session" : undefined,
        },
      }),
    ) as unknown as RunContext["client"]["session"]["get"]
  ...
  it("returns true when appended descendant has agent mismatch and atlas would not continue it", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["root-session", "mismatch-subagent-session"],
      {
        "root-session": "direct",
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "mismatch-subagent-session"
              ? "root-session"
              : undefined,
        },
      }),
  ...
  it("returns true when mismatched descendant was already appended into boulder session_ids", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["root-session", "appended-mismatch-session"],
      {
        "root-session": "direct",
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "appended-mismatch-session"
              ? "root-session"
              : undefined,
        },
      }),
  ...
  it("returns true when appended descendant cannot prove lineage because parent lookup fails", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["root-session", "ses_appended_descendant"],
      {
        "root-session": "direct",
    ...
    ctx.client.session.messages = mock(
      async ({ path }: { path: { id: string } }) => ({
        data:
          path.id === "ses_appended_descendant"
            ? [
                {
                  info: {
                    agent: "atlas",
  ...
  it("returns false when current session is directly tracked among multiple boulder session_ids and has no parent session", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["ses_other_tracked", "ses_direct_tracked"],
      {
  ...
  it("returns false when directly tracked child session has a tracked ancestor and mismatched agent metadata", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["ses_root_tracked", "ses_direct_child"],
      {
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "ses_direct_child" ? "ses_root_tracked" : undefined,
        },
      }),
    ) as unknown as RunContext["client"]["session"]["get"]
    ctx.client.session.messages = mock(
  ...
  it("returns false when latest appended descendant message is compaction but previous real agent still matches atlas", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["root-session", "ses_child_after_compaction"],
      {
        "root-session": "direct",
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "ses_child_after_compaction"
              ? "root-session"
              : undefined,
        },
      }),
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "ses_sqlite_descendant" ? "root-session" : undefined,
        },
      }),
    ) as unknown as RunContext["client"]["session"]["get"]
    ctx.client.session.messages = mock(
  ...
  it("returns false when appended tracked descendant has no persisted messages but in-memory session agent matches atlas", async () => {
    ...
    writeBoulderStateFile(
      directory,
      planPath,
      ["ses_root_tracked", "ses_appended_child"],
      {
        ses_root_tracked: "direct",
    ...
    ctx.client.session.get = mock(
      async ({ path }: { path: { id: string } }) => ({
        data: {
          id: path.id,
          parentID:
            path.id === "ses_appended_child" ? "ses_root_tracked" : undefined,
        },
      }),
    ) as unknown as RunContext["client"]["session"]["get"]
    ctx.client.session.messages = mock(async () => ({
