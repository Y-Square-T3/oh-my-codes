...
describe("resolveMultimodalLookerAgentMetadata", () => {
  ...
  afterEach(() => {
    clearVisionCapableModelsCache()
    ;(
      modelAvailability.fetchAvailableModels as unknown as {
        mockRestore?: () => void
      }
    ).mockRestore?.()
    ;(
      connectedProvidersCache.readConnectedProvidersCache as unknown as {
        mockRestore?: () => void
      }
    ).mockRestore?.()
  })
  ...
  test("returns registered model variant directly without merging from dynamic resolution", async () => {
    ...
    setVisionCapableModelsCache(
      new Map([
        ["openai/gpt-5.4", { providerID: "openai", modelID: "gpt-5.4" }],
      ]),
    )
    spyOn(modelAvailability, "fetchAvailableModels").mockResolvedValue(
