...
} from "../../shared/model-string-parser"
...
export function resolveModelForDelegateTask(input: {
  ...
  const connectedProviders =
    input.availableModels.size === 0 ? readConnectedProvidersCache() : null
  ...
  const explicitHighBaseModel = categoryDefault
    ? getExplicitHighBaseModel(categoryDefault)
    : null
  ...
  if (categoryDefault) {
    ...
    if (input.availableModels.size === 0) {
      const categoryProvider = categoryDefault.includes("/")
        ? categoryDefault.split("/")[0]
        : undefined
      if (
        !connectedProviders ||
        !categoryProvider ||
        connectedProviders.includes(categoryProvider)
      ) {
        return { model: categoryDefault }
      }
    ...
    if (input.availableModels.size === 0) {
      for (const fallbackModel of userFallbackModels) {
        ...
        if (
          ...
          !parsedFallback.providerHint.some((provider) =>
    ...
    if (input.availableModels.size === 0) {
      ...
      for (const entry of fallbackChain) {
        for (const provider of entry.providers) {
          ...
          const match = fuzzyMatchModel(fullModel, input.availableModels, [
            provider,
          ])
          if (match) {
            if (
              explicitHighModel &&
              entry.variant === "high" &&
              match === explicitHighBaseModel
            ) {
              return {
                model: explicitHighModel,
                fallbackEntry: entry,
                matchedFallback: true,
              }
            }
        ...
        const crossProviderMatch = fuzzyMatchModel(
          entry.model,
          input.availableModels,
        )
