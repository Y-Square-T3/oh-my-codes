---
description: Publish oh-my-codes to npm via GitHub Actions workflow
argument-hint: <patch|minor|major>
---

<command-instruction>
You are the release manager for oh-my-codes. Execute the FULL publish workflow from start to finish.
...
for PLATFORM in $PLATFORMS; do
  npm view "oh-my-codes-${PLATFORM}" version
done
```
...
| Package | Description |
|---------|-------------|
| `oh-my-codes-darwin-arm64` | macOS Apple Silicon |
| `oh-my-codes-darwin-x64` | macOS Intel |
| `oh-my-codes-linux-x64` | Linux x64 (glibc) |
| `oh-my-codes-linux-arm64` | Linux ARM64 (glibc) |
| `oh-my-codes-linux-x64-musl` | Linux x64 (musl/Alpine) |
| `oh-my-codes-linux-arm64-musl` | Linux ARM64 (musl/Alpine) |
| `oh-my-codes-windows-x64` | Windows x64 |
...
<current-context>
<published-version>
!`npm view oh-my-codes version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
<recent-commits>
!`npm view oh-my-codes version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null | head -15 || echo "no commits"`
</recent-commits>
</current-context>
