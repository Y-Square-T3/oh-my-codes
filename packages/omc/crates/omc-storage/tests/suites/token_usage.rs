...
pub async fn test_upsert_usage<B: StorageBackend>(backend: &B) {
    ...
    assert_eq!(recent[0].agent, "vscode/agent");
    assert_eq!(recent[0].model, "openai/gpt-4o");
...
pub async fn test_upsert_usage_conflict_updates<B: StorageBackend>(backend: &B) {
    let mut usage = make_usage("vscode/agent", "openai/gpt-4o", 1000, 500, 100, false);
    backend.upsert_usage(&usage).await.unwrap();

    usage.input_tokens = 2000;
    usage.output_tokens = 1000;
    usage.total_tokens = 2000 + 1000 + 100;
    backend.upsert_usage(&usage).await.unwrap();
...
pub async fn test_find_unpushed<B: StorageBackend>(backend: &B) {
    let usage1 = make_usage("vscode/agent", "openai/gpt-4o", 100, 50, 10, false);
    let usage2 = make_usage("cursor/agent", "anthropic/claude", 200, 100, 20, true);
    let usage3 = make_usage("vim/agent", "openai/gpt-4", 300, 150, 30, false);
...
pub async fn test_find_unpushed_with_limit<B: StorageBackend>(backend: &B) {
    for _ in 0..5 {
        let usage = make_usage("client/agent", "provider/model", 100, 50, 10, false);
        backend.upsert_usage(&usage).await.unwrap();
    }
...
pub async fn test_mark_pushed<B: StorageBackend>(backend: &B) {
    let usage1 = make_usage("vscode/agent", "openai/gpt-4o", 100, 50, 10, false);
    let usage2 = make_usage("cursor/agent", "anthropic/claude", 200, 100, 20, false);
...
pub async fn test_cleanup_old_pushed<B: StorageBackend>(backend: &B) {
    ...
    let mut old_usage = make_usage("vscode/agent", "openai/gpt-4o", 100, 50, 10, true);
    old_usage.recorded_at = old;
    backend.upsert_usage(&old_usage).await.unwrap();

    let recent_usage = make_usage("cursor/agent", "anthropic/claude", 200, 100, 20, true);
    backend.upsert_usage(&recent_usage).await.unwrap();

    let unpushed_usage = make_usage("vim/agent", "openai/gpt-4", 300, 150, 30, false);
...
pub async fn test_usage_summary<B: StorageBackend>(backend: &B) {
    let usage1 = make_usage("vscode/agent", "openai/gpt-4o", 1000, 500, 100, false);
    let usage2 = make_usage("cursor/agent", "openai/gpt-4o", 2000, 1000, 200, false);
    let usage3 = make_usage("vim/agent", "anthropic/claude", 3000, 1500, 300, false);
    ...
    let openai_summary = summary.iter().find(|s| s.model == "openai/gpt-4o").unwrap();
