...
pub(crate) async fn find_unpushed(pool: &PgPool, limit: usize) -> Result<Vec<TokenUsage>> {
    let rows: Vec<TokenUsageRow> = sqlx::query_as(
        "SELECT id, client, session_id, message_id, agent, provider_id, model_id, input_tokens, output_tokens, reasoning_tokens, cache_read_tokens, cache_write_tokens, pushed, recorded_at, created_at
         FROM token_usage WHERE pushed = false ORDER BY recorded_at ASC LIMIT $1",
    )
    .bind(limit as i64)
    .fetch_all(pool)
    .await
...
pub(crate) async fn list_recent(
    ...
    let rows: Vec<TokenUsageRow> = if let Some(pushed_val) = pushed {
        ...
        let sql = format!(
            "SELECT id, client, session_id, message_id, agent, provider_id, model_id, input_tokens, output_tokens, reasoning_tokens, cache_read_tokens, cache_write_tokens, pushed, recorded_at, created_at
             FROM token_usage{where_clause} ORDER BY recorded_at DESC LIMIT $1 OFFSET $2",
        );
        sqlx::query_as(&sql)
            .bind(limit as i64)
            .bind(offset as i64)
        ...
        let sql = "SELECT id, client, session_id, message_id, agent, provider_id, model_id, input_tokens, output_tokens, reasoning_tokens, cache_read_tokens, cache_write_tokens, pushed, recorded_at, created_at
             FROM token_usage ORDER BY recorded_at DESC LIMIT $1 OFFSET $2";
        sqlx::query_as(sql)
            .bind(limit as i64)
            .bind(offset as i64)
            .fetch_all(pool)
...
async fn top_clients(pool: &PgPool, cutoff: Option<i64>) -> Result<Vec<UsageGroup>> {
    let sql = format!(
