...
pub(crate) async fn list_providers(pool: &PgPool, account_id: &str) -> Result<Vec<Provider>> {
    let rows: Vec<ProviderRow> = sqlx::query_as(
        "SELECT provider_id, name, env, api, npm, doc, models, account_id, last_fetched_at FROM provider WHERE account_id = $1",
    )
    .bind(account_id)
    .fetch_all(pool)
    .await
...
pub(crate) async fn delete_providers(pool: &PgPool, account_id: &str) -> Result<()> {
    sqlx::query("DELETE FROM provider WHERE account_id = $1")
        .bind(account_id)
        .execute(pool)
        .await
        .map_err(map_err)?;
