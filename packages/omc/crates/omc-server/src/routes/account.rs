...
pub async fn login_handler(
    ...
    let session = state.account_service.login(&req.url).await.unwrap();
    Json(LoginResponse {
...
pub async fn poll_handler(
    ...
    let result = state.account_service.poll(&session).await.unwrap();
    Json(match result {
...
pub async fn active_handler(State(state): State<Arc<DaemonState>>) -> Json<ActiveResponse> {
    let account = state
        .account_service
        .active()
        .await
...
pub async fn list_handler(State(state): State<Arc<DaemonState>>) -> Json<ListResponse> {
    let accounts = state.account_service.list().await.unwrap();
    Json(ListResponse {
        accounts: accounts
            .into_iter()
...
pub async fn remove_handler(
    ...
    state.account_service.remove(&req.account_id).await.unwrap();
    Json(serde_json::json!({ "status": "ok" }))
}
