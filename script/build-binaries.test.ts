...
describe("build-binaries", () => {
  describe("PLATFORMS array", () => {
    ...
    it("has correct directory names for baseline platforms", async () => {
      ...
      const module = await import("./build-binaries.ts")
      const platforms = (
        module as { PLATFORMS: { dir: string; target: string }[] }
      ).PLATFORMS
      ...
      expect(baselinePlatforms.map((p) => p.dir)).toContain(
        "linux-x64-baseline",
      )
      expect(baselinePlatforms.map((p) => p.dir)).toContain(
    ...
    it("has correct binary names for baseline platforms", async () => {
      ...
      const platforms = (
        module as {
          PLATFORMS: { dir: string; target: string; binary: string }[]
        }
      ).PLATFORMS

      // when
      const windowsBaseline = platforms.find(
        (p) => p.target === "bun-windows-x64-baseline",
      )
      const linuxBaseline = platforms.find(
        (p) => p.target === "bun-linux-x64-baseline",
      )

      // then
      expect(windowsBaseline?.binary).toBe("oh-my-codes.exe")
      expect(linuxBaseline?.binary).toBe("oh-my-codes")
    })
