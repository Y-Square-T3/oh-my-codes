...
console.log("=== Publishing oh-my-codes (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `oh-my-codes-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/vibration-autos/oh-my-codes/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(
  ...
  await $`git add package.json assets/oh-my-codes.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
